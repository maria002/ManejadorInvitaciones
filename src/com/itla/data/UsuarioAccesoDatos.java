...
public class UsuarioAccesoDatos {
    ...
    public static List<Usuario> seleccionarTodo() throws SQLException {
        Conexion.conectar();
        ResultSet rs = Conexion.st.executeQuery("SELECT * FROM Usuarios");
        ArrayList<Usuario> usuario = new ArrayList<>();
        while (rs.next()) {
            usuario.add(new Usuario(rs.getInt(1), Conexion.convertirBoolean(rs.getString("activo")), rs.getString("Nombre"), rs.getString("Apellido"), rs.getString("Cuenta"), rs.getString("Clave"), PerfilUsuarioAccesoDatos.SeleccionarPorId(rs.getInt("Id_Perfil_Usuario"))));
        }
    ...
    public static Usuario seleccionarPorId(int id) throws SQLException {
        Conexion.conectar();
        PreparedStatement ps = Conexion.conn.prepareStatement("select * from Usuarios where id = ?");
        ps.setInt(1, id);
        ResultSet rs = ps.executeQuery();
        Usuario usuario = null;
        while (rs.next()) {
            usuario = new Usuario(rs.getInt(1), Conexion.convertirBoolean(rs.getString("activo")), rs.getString("Nombre"), rs.getString("Apellido"), rs.getString("Cuenta"), rs.getString("Clave"), PerfilUsuarioAccesoDatos.SeleccionarPorId(rs.getInt("Id_Perfil_Usuario")));
        }
        Conexion.desconectar();
        return usuario;
    }
    ...
    public static void insertar(Usuario usuario) throws SQLException {
        try {
            Conexion.conectar();
            PreparedStatement ps = Conexion.conn.prepareStatement("Insert into Usuario(Id, Nombre, Apellido, Cuenta, Clave, Activo, ID_PERFIL_USUARIO) values(sec_IdUsuarios.nextVal, ?, ?, ?, ?, ?, ?)");
    ...
    public static void eliminar(int id) throws SQLException {
        try {
            ...
            Conexion.conectar();
            PreparedStatement ps = Conexion.conn.prepareStatement("DELETE FROM Usuarios WHERE id = ?");
            ps.setInt(1, id);
            Conexion.conn.setAutoCommit(false);
            ps.executeUpdate();
            Conexion.conn.commit();
